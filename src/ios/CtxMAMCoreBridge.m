...
- (void) initializeSDKs:(CDVInvokedUrlCommand*)command {
    ...
    [CTXMAMCore setDelegate:(id<CTXMAMCoreSdkDelegate>) [MAMCoreSDK sharedInstance]];
...
- (void) getConfigurationAsNumberForKey:(CDVInvokedUrlCommand*)command {
    ...
    if (err) {
        ...
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:[numberFormatter stringFromNumber:numberResult]];
    }
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}
...
- (void) getConfigurationAsDataForKey:(CDVInvokedUrlCommand*)command {
    ...
    if (err) {
        ...
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArrayBuffer:dataResult];
    }
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}
...
- (void) removeConfigurationForKey:(CDVInvokedUrlCommand*)command {
    ...
    if (err) {
        ...
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:returnVal];
    }
    [self.commandDelegate sendPluginResult:result callbackId:command.callbackId];
}
...
- (void) ctxMAMLog_CriticalErrorFrom:(CDVInvokedUrlCommand*)command {
    ...
    NSString* functionName = [command.arguments objectAtIndex:2];
    int lineNumber = [[command.arguments objectAtIndex:3] intValue];
    NSString* message = [command.arguments objectAtIndex:4];
...
-(void)registerForNotifications:(CDVInvokedUrlCommand*)command {
    ...
    {
        NSString* source = notification.Source;
        NSString* message = notification.Message;
        NSInteger errorCode = notification.Error.code;
        NSString* errorDomain = notification.Error.domain;
        NSString* errorLocalizedDescription = notification.Error.localizedDescription;
        NSString* errorLocalizedFailureReason = notification.Error.localizedFailureReason;
        
        NSDictionary* notificationCallbackDict = @{@"source" : source  , @"message" : message , @"errorCode" : @(errorCode), @"errorDomain" : errorDomain  , @"errorLocalizedDescription" : errorLocalizedDescription , @"errorLocalizedFailureReason" : errorLocalizedFailureReason};        
...
-(void)deRegisterNotifications:(CDVInvokedUrlCommand*)command {
    ...
    if (eventBlock != nil) {
        ...
        CTXMAMLOG_Error(CTXLog_SourceName, @"Attempting to deregister a notification block that has not been resgistered yet.");
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Attempting to deregister a notification block that has not been resgistered yet."];
    }

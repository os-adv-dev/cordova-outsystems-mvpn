...
/**
 * This API can be called to know if the configured CEM server entitles this CitrixMAM SDK app to the current managed user. If entitled, this app can get registered to be a managed app.
 * @return YES, if the app is entitled to the current managed user, otherwise NO.
 */
-(BOOL) isAppEntitledForCurrentManagedUser;
